...
1. Log in to WordPress admin as the user that should own the content
   (e.g. `businessops`).
2. Go to **Users → Profile** (or **Users → Your Profile**).
3. Scroll to **Application Passwords**.
4. Enter a name like `galapagos-wp-publisher` and click **Add New Application
   Password**.
...
6. Put the values in `.env`:
   ...
   ```ini
   WP_BASE_URL=https://www.latintrails.com
   WP_USERNAME=businessops
...
Used only if you ingest documents straight from Drive. The system is locked to
**businessops@latintrails.com** via `GDRIVE_ALLOWED_ACCOUNT` and refuses any
other account.
...
5. First run opens a browser to authorize. Sign in as
   **businessops@latintrails.com**. A `token.json` is cached for next time.
